...
class AIManager:
    ...
    def generate_analysis(self, prompt: str, max_tokens: int, provider: Optional[str] = None) -> Optional[str]:
    ...
    def _record_failure(self, provider_name: str, error_msg: str):
        ...
        if provider_name in self.providers:
            ...
            # Para erros específicos do Gemini (504, 500), aumenta tolerância
            if provider_name == 'gemini' and ('504' in error_msg or '500' in error_msg):
                max_errors = self.providers[provider_name]['max_errors'] + 2
            else:
                max_errors = self.providers[provider_name]['max_errors']

            # Desabilita temporariamente se muitas falhas consecutivas
            if self.providers[provider_name]['consecutive_failures'] >= max_errors:
    ...
    def get_provider_status(self) -> Dict[str, Any]:
        ...
        for name, provider in self.providers.items():
            status[name] = {
                ...
                'enabled': provider.get('enabled', True) # Adiciona status enabled
            }
